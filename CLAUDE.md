...
- `category/<slug>.md` (×7) — página 1 de cada archivo de categoría, escrita a mano, con `category_name` en el front matter; `jekyll-archives` no está en la lista de plugins de GitHub Pages. Las páginas 2, 3... de cada categoría (cuando hay más entradas que `paginate`) se generan aparte — ver «Paginación de los archivos de categoría y etiqueta» más abajo.
- `index.html` (en la raíz, no `.md`) — portada paginada vía `jekyll-paginate` (declarado en `plugins:` de `_config.yml`; `github-pages` no lo activa por sí solo). **No le pongas `permalink:` en el front matter**: al ser el propio fichero plantilla que `jekyll-paginate` clona para cada página siguiente, un permalink explícito se hereda en todas las páginas paginadas y todas acaban resolviendo a la misma URL. `jekyll-paginate` solo sabe paginar esta portada (site.posts al completo): no tiene forma de paginar `site.categories[...]`/`site.tags[...]` por separado, de ahí que los archivos de categoría y etiqueta se paginen a mano (ver más abajo).
...
## Paginación de los archivos de categoría y etiqueta

`jekyll-paginate` (el único plugin de paginación permitido por GitHub Pages) solo sabe paginar la portada (`index.html`, sobre `site.posts`); no tiene forma de paginar `site.categories[...]` o `site.tags[...]` por separado. Para que un archivo de categoría o etiqueta con más de `paginate` entradas (3, ver `_config.yml`) no las muestre todas en una sola página, la paginación de `category/` y `tag/` se resuelve a mano, con páginas físicas adicionales generadas por script — mismo enfoque que las páginas de etiqueta, aplicado también a las de categoría:

- **`scripts/generar_paginas_archivo.rb`** cuenta las entradas de cada categoría y etiqueta en `_posts/*.md` y sincroniza `category/<slug>-pageN.md` y `tag/<slug>-pageN.md` (N = 2, 3...) con `permalink: /category/<slug>/page/N/` y `/tag/<slug>/page/N/` respectivamente: crea las que faltan y borra las que sobran (entrada borrada, categoría/etiqueta quitada de una entrada, o el total ya no llega a otro múltiplo de `paginate`). La página 1 de cada archivo no la toca este script — sigue siendo `category/<slug>.md` (escrita a mano) o `tag/<slug>.md` (generada por `generar_tags.rb`).
- **`_layouts/category.html` y `_layouts/tag.html`** trocean `site.categories[page.category_name]` / `site.tags[page.tag_name]` con los filtros Liquid `slice`/`divided_by` según `page.pagina` (ausente = página 1) y `site.paginate`, sin necesitar el número total de páginas en el front matter. `_includes/archivo-paginacion.html` (mismo patrón visual que la paginación de `home.html`, misma clase CSS `archive-pagination pagination`) calcula los enlaces «anterior/siguiente» y el listado de páginas a partir de una `base` (`/category/<slug>` o `/tag/<slug>`, sin barra final) que cada layout calcula: para categoría, buscando el slug en `site.data.categorias`; para etiqueta, con `{{ page.tag_name | slugify: 'latin' }}`.
- **Cuándo ejecutarlo:** después de añadir, editar o borrar entradas (cambia el `tags:`/`categories:` de cualquiera, o el número total de entradas de una categoría/etiqueta), antes de hacer commit — normalmente junto con `generar_tags.rb`:
  ...
  Revisa el diff de `tag/` y `category/` (páginas nuevas o borradas) y commitéalo junto con la entrada.
