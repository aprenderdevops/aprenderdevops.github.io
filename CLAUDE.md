...
- **Numeración de líneas:** los backticks no la soportan; hace falta la etiqueta Liquid `{% raw %}{% highlight <lenguaje> linenos %}{% endraw %}` ... `{% raw %}{% endhighlight %}{% endraw %}`. Úsala solo cuando el texto del post remita a números de línea concretos («en la línea 2...», «de las líneas 12 a la 17...»); hoy solo la usan dos bloques del post de GitLab con Ansible. Si el bloque además contiene sintaxis Liquid literal, el `{% raw %}...{% endraw %}` va **dentro** del `highlight` (`{% raw %}{% highlight yaml linenos %}{% raw %}{% endraw %}`), no fuera: al revés, la etiqueta `highlight` se imprimiría tal cual en la página.
