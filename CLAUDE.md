...
- `_includes/` — `head.html`, `site-header.html`, `nav-primary.html`, `nav-secondary.html`, `entry.html` (tarjeta reutilizada por `home.html`, `category.html`, `tag.html` y `author.html`), `entry-header.html` (incluye el enlace del autor a `/author/<slug>/`), `entry-footer.html`, `date.html` (formatea fechas en español, ya que Liquid no las localiza), `archive-pagination.html` (paginación compartida por categoría/etiqueta/autor).
